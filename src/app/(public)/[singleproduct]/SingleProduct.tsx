...
//     // Update cart state with quantity check
//     setCart((prevCart) => {
//       // Check if product already exists in cart
//       const existingItemIndex = prevCart.findIndex(item => item._id === id);
...
export default function ProductDetailPage({
  ...
  // Use Search Params For Next Navigation
  const searchParams = useSearchParams();
  const id = productId || searchParams.get("id");
  const name = searchParams.get("name");
  const price = searchParams.get("price");
  ...
  const handleAddToCart = () => {
    ...
    // Update cart state
    setCart((prevCart) => {
      const existingProductIndex = prevCart.findIndex(item => item._id === id);
